...
| `push` | `git push` / `gh pr create` | the **HEAD commit** | repos that push to a remote / open PRs |
...
`bash`, `git`, and `python3` on `PATH` (python3 is used for JSON + safe parsing).
...
| `push` | `git push` / `gh pr create` | **HEAD commit** | مشاريع بتعمل push / PRs |
...
`bash` و`git` و`python3` على الـ `PATH`.
